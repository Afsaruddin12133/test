...
const DetailsProblem = () => {
  ...
  const shortDesc =
    item?.description || (classType ? `This course is about ${classType} development.` : "");
  ...
  }, [courseId, isFree, item?.classType, item?.status, item?.type, setActiveTab]);
      ...
      <div className="max-w-5xl mx-auto p-6">
        <div className="rounded-2xl border border-gray-200 bg-white p-8 text-center">
          ...
          <p className="text-gray-600 mt-2">
            We couldn’t find a class with Subject ID <span className="font-mono">{subject_id}</span>.
          </p>
        </div>
      </div>
    );
            ...
            <>
              {item?.isNegotiable === 1 && (
                <button
                  ...
                  className={`px-5 py-2 rounded-2xl text-sm font-semibold text-white shadow-sm hover:brightness-105 ${
                    activeTab === "negotiate" ? "bg-purple-700" : "bg-black/80"
                  }`}
                >
                  NEGOTIATIONS
                </button>
            ...
            <div className="mt-6 space-y-4">
              {myNegotiation ? (
                <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-3 text-sm text-gray-600">
                      <span>
                        Tutor offer: <span className="font-semibold text-gray-900">৳{fmtBDT(myNegotiation.tutor_offer ?? tutorPrice)}</span>
                      </span>
                      <span>
                        Your offer: <span className="font-semibold text-gray-900">৳{fmtBDT(myNegotiation.student_offer ?? myPrice)}</span>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center gap-2 md:justify-end">
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-xl bg-purple-700 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:brightness-105 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        {negotiationActionId === "student:submit" ? "Sending…" : "Send offer"}
                      </button>
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        className="inline-flex items-center justify-center rounded-xl bg-red-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:brightness-105 disabled:opacity-60 disabled:cursor-not-allowed"
                      >
                        {negotiationActionId === "student:delete" ? "Cancelling…" : "Cancel"}
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="rounded-2xl border border-dashed border-gray-300 bg-white/60 px-5 py-6 text-center text-gray-600">
                  <p className="text-sm">
                    You haven't started a negotiation yet. Submit your price to begin.
                  </p>
                  <div className="mt-4 flex flex-col items-center gap-3">
                    ...
                    >
                      {negotiationActionId === "student:submit" ? "Sending…" : "Submit"}
                    </button>
                  </div>
                </div>
              )}
      ...
      {activeTab === "details" && (
        <div className="mt-6 flex flex-col gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            ...
            <button type="button" onClick={() => setShowMore(true)} className="text-black font-bold hover:underline">
              More
            </button>
          </div>
            ...
            <div className="flex flex-col items-start gap-3 text-gray-600">
              { (Number(item?.duration)) > 0 && (
                <div className="flex items-center gap-2">
                  <AiOutlineClockCircle size={18} /> <span>{formatTotal(Number(item?.duration || 0))}</span>
                </div>
              )}
              <div className="flex items-center gap-2">
          ...
          {subTab === "episodes" ? (
            <>
              <p className="text-gray-700 mt-1">
                There are <span className="font-semibold">{episodes.length}</span> episodes in this course
              </p>
                  ...
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <div className="text-sm text-gray-500">{`EP-${idx + 1}`}</div>
                        <div className="text-gray-900 font-medium">{ep.title || `Class ${idx + 1}`}</div>
                        <div className="text-xs text-gray-500">{`Video- ${getEpisodeMinutes(ep)} Minutes`}</div>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {problemFiles.length > 0 && (
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                    {problemFiles.map((f) => (
                      <div key={f.id} className="border border-gray-200 rounded-2xl bg-white p-3 shadow-sm">
                        {f.preview ? (
                          <img src={f.preview} alt={f.name} className="w-full h-40 object-cover rounded-xl" />
                        ) : (
                        ...
                        <div className="mt-2 text-sm font-medium text-gray-900 truncate">{f.name}</div>
      ...
      {showMore && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative z-10 w-full max-w-4xl rounded-[28px] bg-[#ece7f0] p-6 sm:p-8">
            <h3 className="text-center text-lg font-semibold text-purple-900">Details</h3>
            ...
            <div className="mt-6 grid grid-cols-2 gap-y-4 px-2 text-gray-700">
              ...
              <div className="text-black font-semibold">{item.buyer_count || "0"}</div>

              <div className="font-medium">Institute</div>
              <div className="text-black font-semibold">{item.institute || "Not Available"}</div>
              ...
              <div className="flex flex-wrap gap-2">
                {(item.tags || []).map((t, i) => (
                  <span key={i} className="inline-block rounded-md bg-purple-600 px-3 py-1 text-white text-sm">
                    {t}
                  </span>
                ))}
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-2xl rounded-2xl bg-white p-6 h-[520px] max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">Additional Files</h3>
              <button
                onClick={() => setProblemModalOpen(false)}
