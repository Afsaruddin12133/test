...
const openEpisodeInNewTab = (ep = {}) => {
  try {
    ...
    const src = isYT
      ? `${base}${base.includes("?") ? "&" : "?"}rel=0&modestbranding=1&playsinline=1&origin=${encodeURIComponent(window.location.origin)}`
      : base;
  ...
  return list.map((ep, index) => ({
    ...
    minutes:
      typeof ep.time_duration === "number"
        ? ep.time_duration
        : Number(ep.time_duration) || getEpisodeMinutes({ duration: ep.time_duration }),
    url: ep.url || "",
    isPaid: ep.isPaid,
    hasEnded: ep.has_ended,
    position:
      typeof ep.position === "number" ? ep.position : Number(ep.position) || index,
...
const DetailsRecord = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const eps = await fetchEpisodesByCourseId(subject_id, controller.signal);
  ...
  const negotiationType = Number(item?.classType ?? item?.type ?? DEFAULT_CLASS_TYPE);
  const shortDesc =
    item?.description ||
  ...
  const handleBuyNow = useCallback(async () => {
    ...
    try {
      ...
      toast.success(payload?.message || (isFree ? "Successfully enrolled in the course." : "Successfully purchased the course."));
    } catch (error) {
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-4xl rounded-[28px] bg-[#ece7f0] p-6 sm:p-8">
            ...
            <div className="mt-6 grid grid-cols-2 gap-y-4 px-2 text-gray-700">
              ...
              <div className="text-black font-semibold">{episodes.length}</div>
