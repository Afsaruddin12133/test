...
const Live2 = () => {
  ...
  // Fetch live classes for specific page
  const fetchLiveClasses = async (page = 1) => {
    try {
      setLoading(true);
      setError(null);

      const response = await fetch(`${Base_url}getAllClass?page=${page}&type=2`);
  ...
  // ✅ Map API keys properly
  const liveClass = useMemo(() => {
    return (allClass || [])
      .filter((c) => String(c.type) === "2") // type=2 means "live"
      .map((c) => ({
        id: c.id,
    ...
    <div className="bg-blue-50">
      <div className="max-w-7xl px-4 py-10 mx-auto">
        ...
        <div className="grid gap-2 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          <div className="text-gray-600 text-center">No live classes available.</div>
        )}
      </div>
    </div>
  );
