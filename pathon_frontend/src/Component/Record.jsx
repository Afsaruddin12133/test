...
/* 🔵 Per-tab cache for recorded classes */
let recordData = null;
let recordDataPromise = null;

const Card = ({ ev }) => (
  <div className="rounded-xl bg-white border border-gray-200 shadow-md transition-shadow">
...
const Record = () => {
  // 🔵 init from cache
  const [recordedClass, setRecordedClass] = useState(recordData ?? []);
  ...
  useEffect(() => {
    ...
    if (!recordDataPromise) {
      recordDataPromise = fetch(`${Base_url}getAllClass?page=1&type=1`, {
        ...
        .then((json) => {
          ...
          recordData = normalized; // cache for this tab
          return normalized;
        })
        .catch((err) => {
          throw err;
    ...
    recordDataPromise
      .then((data) => setRecordedClass(data))
      .catch((err) => {
  ...
  return (
    <div className="bg-blue-50 py-8 px-4">
      ...
      ) : recordedClass.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {recordedClass.slice(0, 4).map((ev) => (
            <div key={ev.subject_id} className="px-0">
              <Link
