...
const Record2 = () => {
  ...
  // Fetch record classes for specific page
  const fetchRecordClasses = async (page = 1) => {
    try {
      setLoading(true);
      setError(null);
      
      const response = await fetch(`${Base_url}getAllClass?page=${page}&type=1`);
  ...
  // Initial load
  useEffect(() => {
    fetchRecordClasses(currentPage);
  }, [currentPage]);
  ...
  const recordClass = useMemo(() => {
    return (allClass || [])
      .filter((c) => String(c?.type) === "1") // type=1 => record
      .map((c) => ({
        id: c.id,
      ...
      <div className="max-w-7xl px-4 py-10 mx-auto">
        ...
        {!loading && !error && paginationData.total > 0 && (
          <div className="text-center mb-4 text-gray-600">
            Showing {paginationData.from} to {paginationData.to} of {paginationData.total} record classes
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="text-gray-600 text-center">No record classes available.</div>
        )}
      </div>
    </div>
  );
